	...
	def send_botmenuelem(bot, update, user, menu_elem):
		...
		if menu_elem is None:
			...
			if menu_elem.message_format != MESSAGE_FORMAT.TEXT:
				media_file = menu_elem.telegram_file_code or open(
					menu_elem.media.path, "rb"
				)
		...
		if (
			menu_elem
			and menu_elem.telegram_file_code is None
			and menu_elem.media
			and len(media_codes) > 0
		):
	...
	def send_format_message(
		...
		**telegram_message_kwargs,
		...
		if message_format == MESSAGE_FORMAT.TEXT:
			if text is None:
				raise ValueError(
					f"text message could not be without text: got message_format={message_format}, text={text}"
				)
			...
			raise ValueError(
				f"not group media message with more then 1 file: got message_format={message_format}, media_files_list={media_files_list}"
			)
		elif message_format == MESSAGE_FORMAT.GROUP_MEDIA and (
			len(media_files_list) < 2 or len(media_files_list) > 10
		):
			raise ValueError(
				f"group media message should be with [2, 10] media, got {len(media_files_list)}"
			)
		...
		if is_editing_message:
			...
			if message_format == MESSAGE_FORMAT.TEXT:
				response = bot.edit_message_text(
					text, chat_id, message_id=edit_message_id, **telegram_message_kwargs
				)
			else:
				...
				media_file = input_media(
					media_files_list[0],
					caption=text,
					parse_mode=telegram_message_kwargs.pop(
						"parse_mode", None
					),  # fixme: why pop?
				)
			...
			if message_format == MESSAGE_FORMAT.TEXT:
				response = bot.send_message(chat_id, text, **telegram_message_kwargs)
			else:
				telegram_message_kwargs.pop("disable_web_page_preview", None)
