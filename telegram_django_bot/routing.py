...
def is_command(update: telegram.Update) -> bool:
	return update.message and update.message.text and update.message.text[0] == "/"


class ContextCallbackMessageCommandHandler(Handler):
	def __init__(self, utrl_conf=None, only_utrl=False, **kwargs):
		kwargs["callback"] = lambda x: None
