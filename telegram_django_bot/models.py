...
class TelegramUser(AbstractUser):
	...
	telegram_language_code = models.CharField(
		max_length=16, default="en"
	)  # could be with dialects

	timezone = models.DurationField(default=timezone.timedelta(hours=3))

	current_utrl = models.CharField(
		max_length=64, default="", blank=True
	)  # todo: add verify comparisson current_utrl and current_utrl_context_db/current_utrl_form_db
	current_utrl_code_dttm = models.DateTimeField(null=True, blank=True)
	current_utrl_context_db = models.CharField(
		max_length=4096, default="{}", blank=True
	)
	...
	def current_utrl_form(self):
		if not hasattr(self, "_current_utrl_form"):
			self._current_utrl_form = json.loads(
				self.current_utrl_form_db, cls=TelegramDjangoJsonDecoder
			)
	...
	def current_utrl_context(self):
		if not hasattr(self, "_current_utrl_context"):
			self._current_utrl_context = json.loads(
				self.current_utrl_context_db, cls=TelegramDjangoJsonDecoder
			)
	...
	def save_form_in_db(self, form_name, form_data, do_save=True):
		...
		for key, value in form_data.items():
			if issubclass(value.__class__, models.Model):
				db_value = value.pk
			elif (type(value) in [list, QuerySet]) and all(
	...
	def save(self, *args, **kwargs):
		if self.id is None and self.is_staff:
			...
			logging.warning(
				f"Try to save user without ID. For staff the smallest unused ID will be used: {id_num}"
			)
...
class BotMenuElem(models.Model):
	...
	def get_message(self, language="en"):
		...
		text = (
			get_translate_model.translated_text if get_translate_model else self.message
		)
		return text
		...
		for row_elem in self.buttons:
			...
			for item_in_row in row_elem:
				...
				if (
					...
					and (translate_model := get_translate_model(elem["text"]))
				):
...
class BotMenuElemAttrText(models.Model):
	...
	default_text = models.TextField(help_text=_("The text which should be translate"))
	translated_text = models.TextField(
		null=True, help_text=_("Default_text Translation")
	)


class Trigger(AbstractActiveModel):
	name = models.CharField(max_length=512, unique=True)
	condition_db = models.TextField(
		help_text="""
    {
        seeds: [1, 2, 3, 4, 5],
        'amount': [{
	...
	priority = models.IntegerField(
		default=1, help_text=_("the more topics will be executed first")
	)
