...
class BaseTelegramForm(BaseForm):
	...
	def _init_helper_get_data(self, user, data):
		...
		for model_field in self.base_fields:
			if (
				self.base_fields[model_field].__class__ == ModelMultipleChoiceField
				and model_field in data
			):
				models_from_user = set(data.pop(model_field, []) or [])
				models_from_db = set(curr_data.pop(model_field, []))
				curr_data[model_field] = self._multichoice_intersection(
...
class BaseTelegramModelForm(BaseTelegramForm, BaseModelForm):
		...
		if instance is None:
			...
			for model_field in self.base_fields:
				...
				if hasattr(instance, model_field):
					if (
						self.base_fields[model_field].__class__
						== ModelMultipleChoiceField
					):
						...
						if info_from_user is not None:
							...
							data[model_field] = self._multichoice_intersection(
								get_from_user_pks, get_from_db_pks
							)
						else:
							use_from_db = True
	...
	def save(self, commit=True, is_completed=True):
		...
		if (
			self.is_valid()
			and self.next_field is None
			and (is_completed or self.instance.pk)
		):
			# full valid form
