...
if hasattr(settings, "TELEGRAM_TEST_USER_IDS"):
	...
	logging.warning(
		"TELEGRAM_TEST_USER_IDS is not set in settings. Set it or specify user in each test functions"
	)
...
class TD_TestCase(DJ_TestCase):
	...
	def create_update(
		...
		if user_id is None:
			raise ValueError(
				"the user_id (or TELEGRAM_TEST_USER_IDS) should be set for creating Update model"
			)
