...
from telegram_django_bot.models import (
	MESSAGE_FORMAT,
	BotMenuElem,
	BotMenuElemAttrText,
)
...
class Test_TG_DJ_BOT(TD_TestCase):
	...
	def test_send_format_message(self):
		...
		message, __ = bot.send_format_message(text="simple message", chat_id=user_id)
		...
		message, __ = bot.send_format_message(
			...
			update=update,
			reply_markup=InlineKeyboardMarkupDJ(
				[[InlineKeyboardButtonDJ("test", callback_data="test")]]
			),
	...
	def test_send_botmenuelem(self):
		...
		BotMenuElemAttrText.objects.filter(
			bot_menu_elem=bme, default_text="bme send test", language_code="ru"
	...
	def test_edit_or_send(self):
		...
		message = bot.edit_or_send(None, "test mess", chat_id=user_id)
