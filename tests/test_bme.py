...
class BMEInitData:
	def setup_data(self, with_users=False, with_translation=False):
		...
		if with_translation:
			...
			BotMenuElemAttrText.objects.filter(
				bot_menu_elem=self.bme_start, language_code="ru", default_text="start"
...
class TestBMEModels(TD_TestCase, BMEInitData):
	...
	def test_translation_creating(self):
		...
		self.assertEqual(
			5, BotMenuElemAttrText.objects.filter(language_code="ru").count()
		)
		self.assertEqual(
			4, BotMenuElemAttrText.objects.filter(bot_menu_elem=self.bme_start).count()
		)
		self.assertEqual(
			6, BotMenuElemAttrText.objects.filter(bot_menu_elem=self.bme_2).count()
		)

		self.bme_2.message = "new message"
		self.bme_2.save()
		self.assertEqual(
			8, BotMenuElemAttrText.objects.filter(bot_menu_elem=self.bme_2).count()
		)
	...
	def test_translation_getting(self):
		...
		self.assertEqual(
			should_be.to_dict(),
			InlineKeyboardMarkup(self.bme_start.get_buttons()).to_dict(),
		)

		self.assertEqual("Aloha, world!", self.bme_start.get_message("ru"))
		self.assertEqual(
			should_be.to_dict(),
			InlineKeyboardMarkup(self.bme_start.get_buttons("ru")).to_dict(),
		)
		...
		self.assertEqual(
			should_be.to_dict(),
			InlineKeyboardMarkup(self.bme_start.get_buttons("de")).to_dict(),
		)
...
class TestBMEHandlers(TD_TestCase, BMEInitData):
	...
	def test_command_handler(self):
		message = all_command_bme_handler(
			self.create_update({"text": "/start"}), self.test_callback_context
		)
		self.assertEqual("Aloha, world!", message.text)

		message = all_command_bme_handler(
			self.create_update({"text": "/second"}), self.test_callback_context
		)
	...
	def test_command_handler_special_start(self):
		message = all_command_bme_handler(
			self.create_update({"text": "/start special"}), self.test_callback_context
		)
		self.assertEqual("Aloha, world!", message.text)
	...
	def test_command_handler_no_command(self):
		message = all_command_bme_handler(
			self.create_update({"text": "/not_exist"}), self.test_callback_context
		)
		self.assertEqual(
