...
class CategoryForm(td_forms.TelegramModelForm):
	...
	class Meta:
		...
		labels = {"name": "Category name", "info": "Info"}
...
class OrderForm(td_forms.TelegramModelForm):
	...
	entities = forms.ModelMultipleChoiceField(
		queryset=Entity.objects.all().order_by("-id"), required=False, label="Entities"
	)
	...
	class Meta:
		...
		labels = {"info": "Title", "entities": "Entities"}
