...
from test_app.models import Category, User
from test_app.views import CategoryViewSet, EntityViewSet

from telegram_django_bot.models import ActionLog, BotMenuElem
...
class TestRouterCallbackMessageCommandHandler(TD_TestCase):
	def setUp(self) -> None:
		self.bme = BotMenuElem.objects.create(message="test", command="command")
	...
	def test_command(self):
		...
		res = rc_mch.handle_update(
			update, "dispatcher", "check_result", self.test_callback_context
		)
	...
	def test_callback(self):
		...
		message = self.test_callback_context.bot.send_message(
			chat_id=settings.TELEGRAM_TEST_USER_IDS[0], text="some text"
		)
	...
	def test_callback_user_utrl(self):
		...
		user = User.objects.create(
			id=user_id, username=user_id, current_utrl="cat/cr&name"
		)
