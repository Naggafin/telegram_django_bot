...
from test_app.models import Category, Entity, Order, Size, User
from test_app.views import CategoryViewSet, EntityViewSet, OrderViewSet

from telegram_django_bot.routing import (
...
class TestTelegramViewSet(TD_TestCase):
	...
	def create_category(self):
		category = Category.objects.create(info="Test", name="test")

		return category

	def create_entity(self, category: Category, size: Size):
		entity = Entity.objects.create(
			name="test", category_id=category.pk, author_id=self.user.id, price=322
		)
	...
	def create_order(self, entity: Entity):
		order = Order.objects.create(info="test", status=Order.STATUS_COMPLETED)
	...
	@unittest.skipIf(
		int(telegram.__version__.split(".")[0]) >= 20, "tests do not support async"
	)
	def test_category_permission(self):
		...
		action_delete_category = self.create_update(
			res_message.to_dict(), {"data": f"cat/de&{category.pk}"}
		)
		res_message = self.handle_update(action_delete_category)
		self.assertEqual(
			res_message.text, "Sorry, you do not have permissions to this action."
		)

		action_delete_category2 = self.create_update(
			res_message.to_dict(), {"data": f"cat/de&{category2.pk}"}
		)
		res_message = self.handle_update(action_delete_category2)
		self.assertEqual(
			res_message.text,
			f"Are you sure you want to delete Category  #{category2.pk}?",
		)
		...
		action_update_name = self.create_update(
			res_message.to_dict(), {"data": f"cat/up&{category.pk}&name"}
		)
		...
		action_update_name2 = self.create_update(
			res_message.to_dict(), {"data": f"cat/up&{category2.pk}&name"}
		)
		res_message = self.handle_update(action_update_name2)
		self.assertEqual(
			res_message.text, "Please, fill the field Category name\n\nmax length 128"
		)
	...
	def test_show_list_order_with_foreign_filters(self):
		...
		action_show_list = self.create_update(
			res_message.to_dict(), {"data": f"ord/sl&{entity_2.pk}"}
		)
		...
		self.assertEqual(
			buttons[0][0]["callback_data"], f"ord/se&{order_2.pk}&{entity_2.pk}"
		)
	...
	def test_create_order_v2(self):
		...
		self.assertEqual(
			buttons[0][0]["callback_data"], f"ord/up&&{order_object.pk}&info"
		)

		self.assertEqual(buttons[0][1]["text"], "🔄 Entities")
		self.assertEqual(
			buttons[0][1]["callback_data"], f"ord/up&&{order_object.pk}&entities"
		)
	...
	def test_delete_entity_v2(self):
		...
		action_delete_entity = self.create_update(
			res_message.to_dict(), {"data": f"ent/de&{entity.pk}"}
		)
		res_message = self.handle_update(action_delete_entity)

		deletion_confirmation = self.create_update(
			res_message.to_dict(), {"data": f"ent/de&{entity.pk}&{entity.pk}"}
		)
	...
	def test_change_entity_v2(self):
		...
		action_change_name_entity = self.create_update(
			res_message.to_dict(), {"data": f"ent/up&{entity.pk}&name"}
		)
		res_message = self.handle_update(action_change_name_entity)

		message = res_message.text
		self.assertEqual(
			message, "Please, write the value for field Title \n\nmax length 128"
		)
	...
	def test_show_list_entity_v2(self):
		...
		action_show_list_entity = self.create_update(
			res_message.to_dict(), {"data": "ent/sl"}
		)
	...
	def test_show_elem_entity_v2(self):
		...
		action_show_elem_entity = self.create_update(
			res_message.to_dict(), {"data": f"ent/se&{entity_object.pk}"}
		)
		...
		self.assertEqual(buttons[0][0]["text"], "🔄 Title")
		self.assertEqual(
			buttons[0][0]["callback_data"], f"ent/up&{entity_object.pk}&name"
		)

		self.assertEqual(buttons[0][1]["text"], "🔄 Category")
		self.assertEqual(
			buttons[0][1]["callback_data"], f"ent/up&{entity_object.pk}&category"
		)

		self.assertEqual(buttons[1][0]["text"], "🔄 Sizes")
		self.assertEqual(
			buttons[1][0]["callback_data"], f"ent/up&{entity_object.pk}&sizes"
		)

		self.assertEqual(buttons[1][1]["text"], "🔄 Is visable")
		self.assertEqual(
			buttons[1][1]["callback_data"], f"ent/up&{entity_object.pk}&is_visable"
		)

		self.assertEqual(buttons[2][0]["text"], "🔄 Price")
		self.assertEqual(
			buttons[2][0]["callback_data"], f"ent/up&{entity_object.pk}&price"
		)

		self.assertEqual(buttons[2][1]["text"], "🔄 Author id")
		self.assertEqual(
			buttons[2][1]["callback_data"], f"ent/up&{entity_object.pk}&author_id"
		)
	...
	def test_create_entity_v1(self):
		...
		entity = Entity.objects.get(category_id=category.pk, name="test")
	...
	def test_create_entity_v2(self):
		...
		action_create_entity = self.create_update(
			res_message.to_dict(), {"data": "ent/cr&price&322"}
		)
	...
	def test_create_category_v2(self):
		...
		action_add_category = self.create_update(
			res_message.to_dict(), {"data": f"cat/cr&name&{category_name}"}
		)
		...
		self.assertEqual(
			button_1[0]["callback_data"], f"cat/up&{category_object.pk}&name"
		)
	...
	def test_generate_links(self):
		self.assertEqual(
			"cat/cr&name&hat", self.cvs.gm_callback_data("create", "name", "hat")
		)
	...
	def test_gm_no_elem(self):
		...
		self.assertEqual(
			mess,
			f"The Category {model.pk} has not been found 😱 \nPlease try again from the beginning.",
		)
...
class TestUserViewSet(TD_TestCase):
	...
	def test_show_elem(self):
		...
		action_show_settings = self.create_update(
			res_message.to_dict(), {"data": "us/se"}
		)
		...
		self.assertEqual(
			buttons[0][0]["callback_data"], f"us/up&{self.user.id}&timezone"
		)
		self.assertEqual(buttons[0][1]["text"], "🔄 Language")
		self.assertEqual(
			buttons[0][1]["callback_data"],
			f"us/up&{self.user.id}&telegram_language_code",
		)
	...
	def test_change_language(self):
		...
		self.assertEqual(
			...
			f"us/up&{self.user.id}&telegram_language_code&en",
		)

		self.assertEqual(buttons[1][0]["text"], "German")
		self.assertEqual(
			buttons[1][0]["callback_data"],
			f"us/up&{self.user.id}&telegram_language_code&de",
		)

		self.assertEqual(buttons[2][0]["text"], "Russian")
		self.assertEqual(
			buttons[2][0]["callback_data"],
			f"us/up&{self.user.id}&telegram_language_code&ru",
		)
